...
18. [多模型协同策略](#18-多模型协同策略-) ← **9 条模型选择规则 + Fallback 链 + Token 预算**
## 1. 多 Agent 协作架构
...
> **为什么需要多模型**：不同模型在代码生成、代码审查、Token 效率、中文理解等维度有显著差异。不加区分地用同一模型做所有事，既浪费 Token 也降低质量。

### 18.1 模型选择矩阵（IF-THEN 规则）

| 规则 ID | IF（任务类型） | THEN（推荐模型） | 理由 |
|---------|---------------|-----------------|------|
| M1 | 主 Agent 统筹规划、任务派发、代码评审 | 强推理模型（如 deepseek-reasoner / o3） | 需要深度分析和多步推理 |
| M2 | 子 Agent 执行单一明确修改（1-2 文件，精确行号） | 执行模型（如 deepseek-v4 / claude-sonnet / glm-5） | 不需要大量推理，执行效率优先 |
| M3 | 子 Agent 卡住/绕圈圈超过 3 分钟 | kill 后换模型重派 | 当前模型不适合该任务类型 |
| M4 | 复杂 UI 设计方案的代码实现（生成几十行 HTML/CSS/JS） | 代码生成能力强的模型 | 需要准确生成符合项目约定的代码 |
| M5 | 翻译/多语言文案（lang/*.json） | 中文理解强的轻量模型 | Token 消耗大，不需要强推理 |
| M6 | Lint / 语法检查 / 简单重构 | 最轻量的可用模型 | 确定性任务，不需要创造 |
| M7 | 高 Token 消耗任务（全量搜索、批量修改 >100 文件） | 低成本的模型 | 节省配额 |
| M8 | 首次面对不熟悉的任务类型 | 先试中等模型，卡住换强模型 | 避免浪费强模型 Token |
| M9 | 紧急修复（生产环境问题） | 直接用最强的可用模型 | 可靠优先，成本次之 |
...
首选模型不可用（超时/限流/报错）
    │
    ├── 尝试同梯队替补模型（重试 1 次）
    │     ├─ 强推理梯队: deepseek-reasoner ↔ claude-opus ↔ gpt-4o
    │     ├─ 执行梯队: deepseek-v4 ↔ claude-sonnet ↔ glm-5
    │     └─ 轻量梯队: deepseek-v4-flash ↔ gpt-4o-mini ↔ glm-5-flash
    │
    ├── IF 重试仍然失败
    │   THEN 降级到下一梯队（执行→轻量→轻量偏弱）
    │     └─ 代价: 复杂任务质量可能下降, 主 Agent 需加强 review
    │
    └── IF 所有模型都不可用
        THEN 报告人类, 暂停该任务
```
...
| 任务类型 | 单次 Token 上限 | 超限处理 |
|---------|----------------|---------|
| 主 Agent 派发任务描述 | 3000 字符（约 750 token） | 拆分为更小的子任务 |
| 子 Agent 执行单一文件修改 | 取决于 `runTimeoutSeconds` 配置 | 超时则 kill, 检查是卡住还是慢 |
| 子 Agent 跨文件搜索 | 设置为低 token 限制 | 搜索结果太宽泛则缩小搜索范围 |
| 代码评审（git diff 阅读） | 不限（但分批次进行） | 超过千行的 diff 要求子 Agent 拆分 |

**避免 Token 浪费的规则**:

```text
IF 任务描述超过 3000 字符
THEN → 拆分为多个子任务（每个子任务独立派发）

IF 子 Agent 的 sessions_history 显示连续 3 次相同错误
THEN → 立即 kill, 不等超时（避免继续浪费 Token）

IF 子 Agent 的输出 diff ≤ 3 行（不包含注释和空行）
THEN → 评估是否值得派发子 Agent（可能主 Agent 直接改更快）

IF 同一个子 Agent 因同一原因失败 2 次
THEN → 不再重派, 主 Agent 自行完成

IF 需要在 15 分钟内派发 >6 个子 Agent
THEN → 分批执行, 每批 ≤6 个, 批间等待 30 秒
```

### 18.4 主 Agent vs 子 Agent 的模型选择原则

```text
主 Agent:
- 始终使用强推理模型
- 因为: 任务分解、质量判断、代码评审、异常处理需要深度推理
- 不能降级到轻量模型做评审（会漏掉关键问题）

子 Agent:
- 优先使用执行模型（Token 效率高）
- 因为: 任务是确定的（精确行号、精确修改内容）
- 如果子 Agent 卡住 → 可尝试换同梯队其他模型
- 如果换模型后仍然卡住 → 主 Agent 自行完成

混合策略:
- 同一个任务可以用不同模型分步完成
- 例: 主 Agent（强模型）分析需求 → 子 Agent A（执行模型）写代码 → 子 Agent B（轻量模型）做语法检查
- 例: 子 Agent（执行模型）写代码 → 主 Agent（强模型）做代码评审
```

### 18.5 新增到 §17 决策规则

在「§17.1 任务派发决策」追加以下规则:

| 规则 ID | IF（条件） | THEN（动作） | 来源 |
|---------|-----------|-------------|------|
| F58 | 任务需要深度分析/代码评审/异常处理 | 用强推理模型（如 deepseek-reasoner） | §18.1 M1 |
| F59 | 子 Agent 执行单一明确修改 | 用执行模型（如 deepseek-v4 / glm-5） | §18.1 M2 |
| F60 | 子 Agent 当前模型卡住 3 分钟 | kill, 换同梯队另一模型重派 | §18.1 M3 |
| F61 | 高 Token 消耗任务（全量搜索/改 100+ 文件） | 用低成本轻量模型 | §18.1 M7 |
| F62 | 任务描述超过 3000 字符 | 拆分为多个子任务 | §18.3 |
| F63 | 子 Agent 连续 3 次相同错误 | 立即 kill（不等超时） | §18.3 |
| F64 | 子 Agent 输出 diff ≤ 3 行（有效代码） | 取消子 Agent, 主 Agent 直接改 | §18.3 |
| F65 | 15 分钟内需派发 >6 个子 Agent | 分批执行, 每批 ≤6 个, 批间等 30s | §18.3 |
| F66 | 子 Agent 任务描述预计 >3000 字符 | 拆分为多个子任务再派发 | §18.3 |
| F67 | 主 Agent 做代码评审 | 用强推理模型（不能用轻量模型） | §18.4 |
